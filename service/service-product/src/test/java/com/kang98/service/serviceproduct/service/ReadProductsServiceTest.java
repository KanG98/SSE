...
class ReadProductsServiceTest {
    ...
    void getAllProducts_callMethod_expectedReadProductsResponse() {
        when(productsRepository.findAll()).thenReturn(ProductServiceMockProducts.getAllMockProducts());
    ...
    void getProductsByName_givenEmptyProductName_expectedAllProducts() {
        ...
        when(productsRepository.findAllByName(getProductsRequest.getProductName())).thenReturn(ProductServiceMockProducts.getAllMockProducts());
        ...
        assertAll("Get products by name given empty product name in request",
                () -> assertEquals(expected, actual),
                () -> verify(productsRepository, times(1)).findAllByName(getProductsRequest.getProductName())
        );
    }
    ...
    void getProductsByName_givenProductName_expectedNoProductsList() {
        ...
        when(productsRepository.findAllByName(getProductsRequest.getProductName())).thenReturn(Arrays.asList());
    ...
    void getProductsByName_givenProductNameIgnoreCase_expectedAllProducts() {
        ...
        when(productsRepository.findAllByName(getProductsRequest.getProductName())).thenReturn(Arrays.asList(ProductServiceMockProducts.getMockProduct1()));
